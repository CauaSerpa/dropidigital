<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $tutorial_video = !empty($tutorial_video) ? $tutorial_video : null;

        // Verificar se o ID é válido
        if (!empty($id)) {
            // Query para atualizar os dados
            $sql = "UPDATE tb_routes SET name = :name, title = :title, description = :description, help_page = :help_page, tutorial_video = :tutorial_video WHERE id = :id";
