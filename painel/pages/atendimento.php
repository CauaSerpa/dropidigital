<?php
    ...
    $sql = "SELECT phone, whatsapp, email FROM $tabela WHERE user_id = :user_id";
    ...
    // Separando codigo do pais do restante do numero
    // Use uma expressão regular para encontrar o código do país
    if (preg_match('/^\+(\d+)/', $whatsapp, $matches)) {
        $country_code = $matches[0];
        // Remova o código do país do número
        $phone_number = preg_replace('/^\+\d+\s*/', '', $whatsapp);
    } else {
        $country_code = "+55";
        $phone_number = $shop['whatsapp'];
    }
?>
...
<div class="page__header center">
    ...
    <div class="header__actions">
        <div class="container__button">
            <a href="<?php echo INCLUDE_PATH_DASHBOARD; ?>ajuda/atendimento" class="link-secondary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover text-decoration-none d-flex align-items-center">
...
<form id="myForm" class="position-relative" action="<?php echo INCLUDE_PATH_DASHBOARD ?>back-end/service.php" method="post">
    <div class="card mb-3 p-0">
        <div class="card-header fw-semibold px-4 py-3 bg-transparent">Informações de atendimento</div>
        <div class="card-body row px-4 py-3">
            <div class="col-sm-6 mb-3">
                <label for="phone" class="form-label small">Telefone *</label>
                <input type="text" class="form-control" name="phone" id="phone" placeholder="(00) 00000-0000" value="<?php echo $shop['phone']; ?>" required>
            </div>
            <div class="col-sm-6 mb-3">
                <div class="row">
                    <label for="phone-number" class="form-label small">Número do WhatsApp</label>
                    <div class="col-md-2">
                        <input type="text" class="form-control" name="country_code" id="country-code" placeholder="+55" value="+55" value="<?php echo $country_code; ?>">
                    </div>
                    <div class="col-md-10">
                        <input type="text" class="form-control" name="phone_number" id="phone-number" placeholder="(00) 00000-0000" value="<?php echo $phone_number; ?>">
                    </div>
            ...
            <div class="col-sm-6 mb-3">
                ...
                <input type="text" class="form-control" name="email" id="email" placeholder="seu-email@mail.com" value="<?php echo $shop['email']; ?>" required>
            </div>
        </div>
    </div>
    ...
    <div class="save-button bg-white px-6 py-3 align-item-right" id="saveButton" style="position: fixed;width: calc(100% - 78px);left: 78px;bottom: 0px;z-index: 99999; display: none;">
        <div class="container-save-button container fw-semibold bg-transparent d-flex align-items-center justify-content-between">
            <a href="<?php echo INCLUDE_PATH_DASHBOARD; ?>atendimento" class="text-decoration-none text-reset">Cancelar</a>
