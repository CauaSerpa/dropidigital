...
<?php
        ...
        $sql = "SELECT * FROM $tabela WHERE status = :status AND domain != :domain ORDER BY id DESC";
...
<div class="page__header center">
    <div class="header__title">
        <h2 class="title">Domínios próprios</h2>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th class="small">Tempo Restante</th>
                    ...
                    foreach ($domains as $domain) {
                        ...
                        foreach ($shops as $shop) {
                            ...
                            foreach ($users as $user) {
                                ...
                                // Hora inicial
                                $horaInicial = new DateTime($domain['register_date']);

                                // Tempo em horas
                                $tempoEmHoras = 48;

                                // Clonar a hora inicial para não modificar a original
                                $horaFinal = clone $horaInicial;

                                // Adicionar o tempo em horas
                                $horaFinal->add(new DateInterval("PT{$tempoEmHoras}H"));
                            
                                // Hora atual
                                $horaAtual = new DateTime();
                            
                                // Calcular a diferença entre a hora final e a hora atual
                                $diferenca = $horaFinal->diff($horaAtual);

                                // Lógica para formatar a saída
                                if ($diferenca->d >= 1) {
                                    $time_left = $diferenca->format('%d dia');
                                } else if ($diferenca->h >= 1) {
                                    $time_left = $diferenca->format('%H horas');
                                } elseif ($diferenca->i >= 1) {
                                    $time_left = $diferenca->format('%I minutos');
                                } else {
                                    $time_left = $diferenca->format('%S segundos');
                                }
