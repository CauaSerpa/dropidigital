...
<div class="page__header center">
    <div class="header__title">
        ...
        <p class="shop-counter"><?php echo $countWebsites; echo ($countWebsites == 0 || $countWebsites == 1) ? ' site' : ' sites'; ?></p>
    </div>
    <div class="header__actions">
        <div class="container__button">
            <a href="<?php echo INCLUDE_PATH_DASHBOARD; ?>criar-site-pronto" class="button button--flex new text-decoration-none">+ Criar Site Pronto</a>
        </div>
    </div>
</div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th class="small">Nome do Site</th>
                    ...
                    // Conta o numero de sites exibidos  
                    $countWebsites = $stmt->rowCount();

                    // Loop através dos resultados e exibir todas as colunas
                    foreach ($websites as $site) {
                        //Formatacao preco
                        // $price = str_replace(',', '.', str_replace('.', '', $site['price']));
                        $preco = $site['price'];

                        // Transforma o número no formato "R$ 149,90"
                        $price = "R$ " . number_format($preco, 2, ",", ".");

                        $price = ($site['without_price'] == 1) ? "--" : $price;

                        echo '
                            <tbody>
                                <tr>
                                    <td scope="row">
                                        <input class="form-check-input itemCheckbox" type="checkbox" name="selected_ids[]" value="' . $site['id'] . '" id="defaultCheck2">
                                    </td>
                                    <td title="' . $site['name'] . '">
                        ';
                        ...
                        echo '
                                    ...
                                    <td title="' . strip_tags($site['description']) . '">' . strip_tags($site['description']) . '</td>
                        ...
                        ';
                                ...
                                // Preparar e executar a consulta
                                $stmt = $conn_pdo->prepare($sql);
                                $stmt->bindParam(':id', $site['plan_id']);
                                $stmt->execute();
                                ...
                                if ($plan) {
                                    echo "<td title='" . $plan['name'] . "'>";
                                    echo $plan['name'];
                                    echo "</td>";
                                }
                                    
                        echo '
                                    <td title="' . date("d/m/Y", strtotime($site['date_create'])) . '">' . date("d/m/Y", strtotime($site['date_create'])) . '</td>
                                    <td>
                        ';
                                ...
                                if ($domain) {
                                    ...
                                    echo '<a href="' . $url . '" class="btn btn-secondary">';
                        ...
                        echo '
                                        ...
                                        <a href="' . INCLUDE_PATH_DASHBOARD . 'excluir-site-pronto?id=' . $site['id'] . '" class="btn btn-danger">
            ...
            <div class="center">
                <div class="left">
                    <div class="container__button">
                        ...
                        <label>Sites Prontos por página</label>
                    </div>
                </div>
                <div class="right grid">
                    <div class="controller">
                        <?php
                            $numWebsites = ceil($countWebsites / $limite);
                            // Garante que numWebsites seja pelo menos 1
                            $numWebsites = max(1, $numWebsites);

                            $search = isset($_GET['search']) ? "&search=" . $_GET['search'] : "";

                            for ($i = 1; $i <= $numWebsites; $i++) {
            ...
            <?php
                    ...
                    echo '
                            <div class="p-5 text-center">
                                <i class="bx bx-windows" style="font-size: 3.5rem;"></i>
                                <p class="fw-semibold mb-4">Não existe nenhum site pronto cadastrado!</p>
                            </div>
                        ';
                }
            ?>
